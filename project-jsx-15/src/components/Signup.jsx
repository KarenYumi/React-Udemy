...
function singupAction(prevFormState, formData) {
  ...
  if (!isNotEmpty(password) || !hasMinLength(password, 6)) {
    errors.push("reveja a sua senha");
  }
  ...
  if (!isNotEmpty(firstName) || !isNotEmpty(lastName)) {
    errors.push("Coloque o nome e sobrenome");
  }

  if (!isNotEmpty(role)) {
    errors.push("Coloque um role");
  }

  if (!terms) {
    errors.push("è obrigatório este campo");
  }

  if (acquisitionChannel.length === 0) {
    errors.push("Selecione pelo menos");
  }
